...
class ManageProspectDosenPage extends StatefulWidget {
  @override
  _ManageProspectDosenPageState createState() =>
      _ManageProspectDosenPageState();
}

class _ManageProspectDosenPageState extends State<ManageProspectDosenPage> {
  List<PendingBookingProdiModel> _prospects = [];
  List<PendingBookingProdiModel> _filteredProspects = [];
  ...
  Future<void> selectAllSchedule() async {
    try {
      String response =
          await ds.selectAll(token, project, 'pending_booking_prodi', appid);
      List<dynamic> data = jsonDecode(response);
      setState(() {
        _prospects =
            data.map((e) => PendingBookingProdiModel.fromJson(e)).toList();
        _applyFilters();
      });
    } catch (error) {
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () {
                        ...
                        _updateStatus(_prospects.indexOf(prospect), 'Canceled',
    ...
    try {
      ...
      if (success) {
        if (newStatus != 'Canceled') {
          ...
          if (startDateTime.isAfter(workingStartTime) &&
              startDateTime.isBefore(workingEndTime)) {
        ...
        if (newStatus == 'Approved') {
          await ds.insertBooking(appid, date, startTime, endTime, 'Pending',
              desc, user, room, statApprove, '', '');
        }
